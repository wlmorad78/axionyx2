...
if (!$customerId || !$date) {
    echo "Usage: php report_customer_sales.php <customer_id> <date>\n";
    echo "Example: php report_customer_sales.php 697 2026-09-01\n";
    exit(1);
}
...
if ($items->isEmpty()) {
    echo "No sales found for this customer on this date.\n";
    exit(0);
}
...
echo "\n";
echo "=== Customer Sales Report: " . $first->customer_name . " (" . $first->customer_code . ") ===\n";
echo "Date: " . $date . " | Rep: " . $first->rep_name . "\n";
echo $separator . "\n";
printf("%-16s %-8s %-10s %-25s %-10s %-8s %-8s %-10s %-10s %-10s %-12s\n",
    'Invoice', 'Time', 'Code', 'Item Name', 'Unit', 'Qty', 'Bonus', 'Price', 'Disc', 'Tax', 'Net Amount');
echo $separator . "\n";
