...
$invoiceNos = array_slice($argv, 1);

if (empty($invoiceNos)) {
    echo "Usage: php report_multi_invoice_items.php <invoice1> <invoice2> ...\n";
    echo "Example: php report_multi_invoice_items.php 260901-2026-010 260901-2025-026 260901-2025-037\n";
    exit(1);
}
...
$items = DB::table('sales_invoices')
    ...
    ->whereIn('sales_invoices.invoice_no', $invoiceNos)
    ...
    ->select(
        ...
        'sales_invoices.net_total as invoice_total',
...
echo "\n";
echo "=== Invoice Items Report ===\n";
echo $separator . "\n";
printf("%-16s %-16s %-25s %-10s %-8s %-8s %-10s %-10s %-10s %-12s\n",
    'Invoice', 'Customer', 'Item Name', 'Unit', 'Qty', 'Bonus', 'Price', 'Disc', 'Tax', 'Net Amount');
echo $separator . "\n";
...
foreach ($items as $item) {
    ...
    printf("%-16s %-16s %-25s %-10s %-8s %-8s %-10s %-10s %-10s %-12s\n",
        $item->invoice_no ?? '—',
        mb_substr($item->customer_name, 0, 15) ?? '—',
