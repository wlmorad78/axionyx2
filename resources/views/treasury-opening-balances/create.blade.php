...
@section('title', 'إضافة رصيد افتتاحي للخزنة - Axionyx ERP')
@section('page_title', 'إضافة رصيد افتتاحي للخزنة')
@section('page_subtitle', 'تحديد الرصيد الافتتاحي لأحدى الخزنة')
...
<article class="panel">
    ...
    <form method="POST" action="{{ route('treasury-opening-balances.store') }}">
        @csrf
            ...
            <div>
                ...
                <select name="treasury_id" required style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:8px;padding:10px 12px;color:#fff;font-size:13px;">
                    ...
                    @foreach($treasuries as $treasury)
                        <option value="{{ $treasury->id }}" {{ $selectedTreasury == $treasury->id ? 'selected' : '' }}>
