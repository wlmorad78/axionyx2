...
@section('title', 'إضافة رصيد افتتاحي - Axionyx ERP')
@section('page_title', 'إضافة رصيد افتتاحي')
@section('page_subtitle', 'إنشاء قيد أرصدة افتتاحية جديد')

@section('content')
<article class="panel">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
        <h2 style="margin:0;">إنشاء قيد أرصدة افتتاحية</h2>
    ...
    <form method="POST" action="{{ route('opening-balances.store') }}" id="balanceForm">
        @csrf
        <input type="hidden" name="balance_type" id="balanceType" value="{{ $balanceType }}">
        ...
        <div style="margin-bottom:16px;">
            ...
            <textarea name="notes" rows="2" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:8px;padding:10px 12px;color:#fff;font-size:13px;" placeholder="ملاحظات اختيارية..."></textarea>
        </div>

        <div style="display:flex; gap:8px; margin-bottom:16px;">
            <button type="button" onclick="setType('cash')" id="btnCash" class="btn" style="font-size:12px;padding:8px 16px;{{ $balanceType === 'cash' ? 'background:rgba(34,197,94,0.2);border-color:var(--primary);' : '' }}">الخزنة</button>
            <button type="button" onclick="setType('accounts')" id="btnAccounts" class="btn" style="font-size:12px;padding:8px 16px;{{ $balanceType === 'accounts' ? 'background:rgba(56,189,248,0.2);border-color:var(--accent);' : '' }}">البنك</button>
            <button type="button" onclick="setType('inventory')" id="btnInventory" class="btn" style="font-size:12px;padding:8px 16px;{{ $balanceType === 'inventory' ? 'background:rgba(168,85,247,0.2);border-color:#a855f7;' : '' }}">المنتجات</button>
            <button type="button" onclick="setType('suppliers')" id="btnSuppliers" class="btn" style="font-size:12px;padding:8px 16px;{{ $balanceType === 'suppliers' ? 'background:rgba(245,158,11,0.2);border-color:var(--warn);' : '' }}">الموردين</button>
            <button type="button" onclick="setType('customers')" id="btnCustomers" class="btn" style="font-size:12px;padding:8px 16px;{{ $balanceType === 'customers' ? 'background:rgba(236,72,153,0.2);border-color:#ec4899;' : '' }}">العملاء</button>
            <button type="button" onclick="setType('assets')" id="btnAssets" class="btn" style="font-size:12px;padding:8px 16px;{{ $balanceType === 'assets' ? 'background:rgba(99,102,241,0.2);border-color:#6366f1;' : '' }}">الأصول</button>
        </div>
                ...
                <table id="linesTable">
                    <thead>
                        <tr>
                            ...
                            <th id="colAccount" style="display:{{ in_array($balanceType, ['accounts', 'cash', 'assets']) ? '' : 'none' }}">الحساب</th>
                            <th id="colCustomer" style="display:{{ $balanceType === 'customers' ? '' : 'none' }}">العميل</th>
                            <th id="colSupplier" style="display:{{ $balanceType === 'suppliers' ? '' : 'none' }}">المورد</th>
                            <th id="colItem" style="display:{{ $balanceType === 'inventory' ? '' : 'none' }}">الصنف</th>
                            <th id="colWarehouse" style="display:{{ $balanceType === 'inventory' ? '' : 'none' }}">المخزن</th>
                            <th id="colUnit" style="display:{{ $balanceType === 'inventory' ? '' : 'none' }}">الوحدة</th>
                            <th id="colQty" style="display:{{ $balanceType === 'inventory' ? '' : 'none' }}">الكمية</th>
                            <th id="colCost" style="display:{{ $balanceType === 'inventory' ? '' : 'none' }}>تكلفة الوحدة</th>
                            <th>مدين</th>
                            <th>دائن</th>
                            <th>البيان</th>
                            <th style="width:40px;"></th>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px;background:rgba(17,24,39,0.92);border:1px solid var(--line);border-radius:12px;">
            ...
            <button type="submit" class="btn primary" style="font-size:14px;padding:10px 24px;" id="submitBtn">حفظ القيد</button>
        </div>
    </form>
</article>
...
function addLine() {
    ...
    if (['accounts', 'cash', 'assets'].includes(type)) {
        cells += `<td><select name="lines[${lineIndex}][account_id]" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;">
            ...
            ${accounts.map(a => `<option value="${a.id}">${a.code} - ${a.name_ar}</option>`).join('')}
        </select></td>`;
    }
    ...
    if (type === 'customers') {
        cells += `<td><select name="lines[${lineIndex}][customer_id]" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;">
            ...
            ${customers.map(c => `<option value="${c.id}">${c.code} - ${c.name_ar}</option>`).join('')}
        </select></td>`;
    }
    ...
    if (type === 'suppliers') {
        cells += `<td><select name="lines[${lineIndex}][supplier_id]" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;">
            ...
            ${suppliers.map(s => `<option value="${s.id}">${s.supplier_code} - ${s.name_ar}</option>`).join('')}
        </select></td>`;
    }
    ...
    if (type === 'inventory') {
        cells += `<td><select name="lines[${lineIndex}][item_id]" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;">
            ...
            ${items.map(i => `<option value="${i.id}">${i.code} - ${i.name_ar}</option>`).join('')}
        </select></td>`;
        cells += `<td><select name="lines[${lineIndex}][warehouse_id]" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;">
            ...
            ${warehouses.map(w => `<option value="${w.id}">${w.code} - ${w.name_ar}</option>`).join('')}
        </select></td>`;
        cells += `<td><select name="lines[${lineIndex}][unit_id]" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;">
            ...
            ${units.map(u => `<option value="${u.id}">${u.code} - ${u.name_ar}</option>`).join('')}
        </select></td>`;
        cells += `<td><input type="number" name="lines[${lineIndex}][qty]" step="0.01" min="0" value="0" style="width:70px;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;" onchange="updateTotals()"></td>`;
        cells += `<td><input type="number" name="lines[${lineIndex}][unit_cost]" step="0.01" min="0" value="0" style="width:90px;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;" onchange="updateTotals()"></td>`;
    }

    cells += `<td><input type="number" name="lines[${lineIndex}][debit]" step="0.01" min="0" value="0" style="width:100px;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;" onchange="updateTotals()"></td>`;
    cells += `<td><input type="number" name="lines[${lineIndex}][credit]" step="0.01" min="0" value="0" style="width:100px;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;" onchange="updateTotals()"></td>`;
    cells += `<td><input type="text" name="lines[${lineIndex}][description]" style="width:120px;background:#0b1220;border:1px solid var(--line);border-radius:6px;padding:6px;color:#fff;font-size:12px;" placeholder="البيان"></td>`;
