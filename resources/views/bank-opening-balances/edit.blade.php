...
<article class="panel">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
        ...
        <div style="display:flex; gap:8px;">
            <a href="{{ route('bank-opening-balances.show', $bankOpeningBalance->id) }}" class="btn" style="font-size:13px;">عرض التفاصيل</a>
    ...
    <form method="POST" action="{{ route('bank-opening-balances.update', $bankOpeningBalance->id) }}">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="bank_account_id" required style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:8px;padding:10px 12px;color:#fff;font-size:13px;">
                    ...
                    @foreach($bankAccounts as $account)
                        <option value="{{ $account->id }}" {{ $bankOpeningBalance->bank_account_id == $account->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="fiscal_year_id" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:8px;padding:10px 12px;color:#fff;font-size:13px;">
                    ...
                    @foreach($fiscalYears as $year)
                        <option value="{{ $year->id }}" {{ $bankOpeningBalance->fiscal_year_id == $year->id ? 'selected' : '' }}>
                            {{ $year->year_code }}
                        </option>
                    @endforeach
                </select>
        ...
        <div style="margin-bottom:16px;">
            ...
            <input type="number" name="opening_balance" step="0.01" min="0" value="{{ old('opening_balance', $bankOpeningBalance->opening_balance) }}" required
        ...
        <div style="margin-bottom:16px;">
            ...
            <textarea name="notes" rows="3" style="width:100%;background:#0b1220;border:1px solid var(--line);border-radius:8px;padding:10px 12px;color:#fff;font-size:13px;resize:vertical;" placeholder="أدخل ملاحظات اختيارية">{{ old('notes', $bankOpeningBalance->notes) }}</textarea>
