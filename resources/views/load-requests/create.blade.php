...
@section('title', 'طلب تحميل جديد - Axionyx ERP')
@section('page_title', 'طلب تحميل بضاعة جديد')
@section('page_subtitle', 'إضافة أصناف للتحميل على جهاز المندوب')

@section('content')
<article class="panel">
    <h2>بيانات طلب التحميل</h2>
    ...
    <form method="POST" action="{{ route('load-requests.store') }}" id="loadRequestForm">
        @csrf
        ...
        <h3 style="margin-bottom:12px;">الأصناف المطلوبة</h3>
        ...
        <div style="margin-top:24px; display:flex; gap:12px; justify-content:flex-end;">
            <a href="{{ route('load-requests.index') }}" class="btn">إلغاء</a>
            <button type="submit" class="btn primary" id="submitBtn">إرسال طلب التحميل</button>
        </div>
    </form>
</article>
