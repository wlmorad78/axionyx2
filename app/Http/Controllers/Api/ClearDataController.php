...
class ClearDataController extends Controller
{
    ...
    public function clearGroup(string $group)
    {
        if (!isset($this->groups[$group])) {
            return response()->json(['message' => 'مجموعة غير موجودة'], 404);
        }
    ...
    public function clearTable(string $table)
    {
        ...
        if (!$found) {
            return response()->json(['message' => 'جدول غير موجود'], 404);
        }

        if (!DB::getSchemaBuilder()->hasTable($table)) {
            return response()->json(['message' => "الجدول {$table} غير موجود في قاعدة البيانات"], 404);
        }
